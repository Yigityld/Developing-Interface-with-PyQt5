...
class Apps(QMainWindow):
    def __init__(self):
        ...
        self.UP_ARROW_thread = None
        self.DOWN_ARROW_thread = None
        ...
        slider_labels = ["Hız", "Left Motor", "Right Motor", "Head", "Palet"]
        ...
        for label_name in slider_labels:
            ...
            self.slider_label = QLabel(label_name, self)
    ...
    def update_slider(self, slider_name, value):
        for slider, name in self.slider_references:
            if name == slider_name:
                scaled_value = float(value) * 10
                slider.setValue(int(scaled_value))
                print(f"{slider_name} slidesr değeri değiştirildi. Yeni değer: {int(scaled_value)}")
                break

    def update_slider_press_repeatedly_UP_ARROW(self):
        while self.UP_ARROW_pressed:
            self.update_slider_press("Head", 1)
            time.sleep(0.2)

    def update_slider_press_repeatedly_DOWN_ARROW(self):
        while self.DOWN_ARROW_pressed:
            self.update_slider_press("Head", -1)
            time.sleep(0.2)

    def update_slider_press_repeatedly(self):
        while self.R1_pressed:
            self.update_slider_press("Palet", 1)
            time.sleep(0.1)

    def update_slider_press_repeatedly_L1(self):
        while self.L1_pressed:
            self.update_slider_press("Palet", -1)
            time.sleep(0.1)

    def update_slider_press(self, slider_name, value):
        for slider, name in self.slider_references:
            if name == slider_name:
                self.start_value =slider.value()
                self.start_value += value
                slider.setValue(int(self.start_value))
                print(f"{slider_name} slidesr değeri değiştirildi. Yeni değer: {int(self.start_value)}")
                        ...
                        if event.type == pygame.JOYBUTTONDOWN:
                            ...
                            if button_id == X_BUTTON:
                                ...
                                if not self.L1_pressed:
                                    ...
                                    self.L1_thread = threading.Thread(target=self.update_slider_press_repeatedly_L1)
                                    self.L1_thread.start()
                                print("L1 pressed")
                            elif button_id == R1_BUTTON:
                                if not self.R1_pressed:
                                    self.R1_pressed = True
                                    self.R1_thread = threading.Thread(target=self.update_slider_press_repeatedly)
                                    self.R1_thread.start()
                                print("R1 pressed")
                            elif button_id == UP_ARROW_BUTTON:
                                if not self.UP_ARROW_pressed:
                                    self.UP_ARROW_pressed = True
                                    self.UP_ARROW_thread = threading.Thread(
                                        target=self.update_slider_press_repeatedly_UP_ARROW)
                                    self.UP_ARROW_thread.start()
                                print("Up arrow pressed")

                            elif button_id == DOWN_ARROW_BUTTON:
                                if not self.DOWN_ARROW_pressed:
                                    self.DOWN_ARROW_pressed = True
                                    self.DOWN_ARROW_thread = threading.Thread(
                                        target=self.update_slider_press_repeatedly_DOWN_ARROW)
                                    self.DOWN_ARROW_thread.start()
                            ...
                            if button_id == X_BUTTON:
                                ...
                                self.UP_ARROW_pressed = False
                                if self.UP_ARROW_thread:
                                    self.UP_ARROW_thread.join()
                                print("Up arrow released")

                            elif button_id == DOWN_ARROW_BUTTON:
                                self.DOWN_ARROW_pressed = False
                                if self.DOWN_ARROW_thread:
                                    self.DOWN_ARROW_thread.join()
                            ...
                            if axis == LEFT_ANALOG_X:
                                ...
                                value *= -1
                                value = f"{value:.3f}"
                                self.update_slider("Left Motor", value)
                                ...
                                value *= -1
                                value = f"{value:.3f}"
                                self.update_slider("Right Motor", value)
